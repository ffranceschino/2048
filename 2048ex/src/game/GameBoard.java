...
public class GameBoard {
	...
	board = new Tile[ROWS][COLS];
        //db.setCurrentUsername(DB.getCurrentUsername());
    ...
    public void update() {
        if(mainMenuPanel.getDiff()) {
            time=getTime();
            scores.setDifficulty(time);
        }
        else {
            time=scores.getDifficulty();
        }
	...
	for (int row = 0; row < ROWS; row++) {
		...
		if(board[row][col].getValue() >= 256) return true;
            }
	}
	return false;
    }
	...
	lBoard.saveScores();
        ...
        if (scores.getCurrentScore() > DB.getInstance().getBestScore(DB.getCurrentUsername())) {
            DB.getInstance().saveBestScore(scores.getCurrentScore());
        }
	}
	this.dead = dead;
    }
